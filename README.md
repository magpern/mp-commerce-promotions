...
- **Demo types supported:** conditions **`minimum_subtotal`**, **`product_quantity`**, **`category_quantity`**, **`logged_in`**, **`first_order`**, **`customer_role`**, **`billing_country`** (ISO codes from Woo billing), **`customer_email_domain`** (email domain from Woo/user), **`customer_redemption_count`** (logged-in redemption total from DB); actions **`percentage_discount`**, **`fixed_amount_discount`**, **`free_shipping`** (MVP: negative fee = shipping total), and **`cheapest_item_discount`** (BOGO groundwork: cheapest eligible units by category or product list; applied as negative fee — does not add free products or change line prices). **Simple Rule Builder v0** covers customer/location conditions, simple discounts, free shipping, and **cheapest item discount**. **`PromotionPlanner`** selects promotions per application rules; each selected promotion uses only its **first** supported action. **Fixed amount** / **percentage** / **cheapest item** discounts are clamped per fee and **cumulative total ≤ cart subtotal**. Storefront BOGO checks: [docs/manual-cheapest-item-test.md](docs/manual-cheapest-item-test.md). **`free_shipping`** requires browser checkout verification when shipping methods apply.
- **Admin cart preview** — **Edit promotion** can run **`PromotionEvaluator`** against the current session cart and show the **promotion plan** table (selected / skipped reason); **admin-only**, **no persistence**, **does not add cart fees**.
- **Storefront cart fees (v1)** — **`CartPromotionApplier`** runs on **`woocommerce_cart_calculate_fees`** (priority **20**): **`PromotionPlanner`** on **`find_active()`** promotions, then one negative fee per **selected** promotion (first supported action only; subtotal cap). Session **`mp_cp_applied_promotion`** includes **`applied_promotions[]`** and **`total_discount_amount`**. **`OrderPromotionRecorder`** writes one **redemption** per **`(order_id, promotion_id)`** (idempotent), **`_mp_cp_applied_promotions`** JSON, and reverses all on cancel/refund/trash/delete. **Code-linked** path skips automatic promotions. Toggle fees with **`mp_cp_enable_cart_discounts`** (default `true`).
...
- **Cart & Checkout Blocks** — **not** declared compatible yet. The plugin uses cart fees and the standard coupon field; block checkout behavior should be verified manually before adding a `cart_checkout_blocks` declaration.
