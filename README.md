...
- **Codes** — manual entry or batch generation (max **1,000** per batch); full codes shown once (copy or CSV download before refresh); batch detail view shows metadata and **last 4** only (no CSV re-download). Per-code **disable / enable / mark expired** in admin (no delete; expired cannot return to active). Virtual coupon amount is **0** (discount is the cart fee). No PDF/email export.
