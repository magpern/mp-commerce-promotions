...
- **`PromotionCodeBatch`** / **`PromotionCodeBatchRepository`** / **`PromotionCodeBatchGenerator`** — admins generate up to **1,000** unique codes per batch on the promotion edit screen. Codes use **`PREFIX-RANDOM`** (optional prefix) or **`RANDOM`** (12+ character cryptographically secure segment; excludes ambiguous **O/0/I/1**). Full codes are shown **once** in admin after generation (textarea + **Download CSV** on the same response); plain codes are **not** stored in the database (only **hashes** and **last 4**). CSV columns: `code`, `promotion_id`, `batch_id`, `generated_at` — download must happen **before** leaving or refreshing the page. Audited as **`promotion_code.batch_generated`**. **No PDF or email** yet.
