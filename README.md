...
- **`PromotionCodeBatch`** / **`PromotionCodeBatchRepository`** / **`PromotionCodeBatchGenerator`** — admins generate up to **1,000** unique codes per batch on the promotion edit screen. Each generated code row stores **`batch_id`** for traceability. Codes use **`PREFIX-RANDOM`** (optional prefix) or **`RANDOM`** (12+ character cryptographically secure segment; excludes ambiguous **O/0/I/1**). Full codes are shown **once** in admin after generation (textarea + **Download CSV** on the same response); plain codes are **not** stored in the database (only **hashes**, **last 4**, and **batch_id**). CSV columns: `code`, `promotion_id`, `batch_id`, `generated_at` — download must happen **before** leaving or refreshing the page. Audited as **`promotion_code.batch_generated`**. **No PDF or email** yet.
...
- **Codes** — manual entry or batch generation (max **1,000** per batch); full codes shown once (copy or CSV download before refresh); virtual coupon amount is **0** (discount is the cart fee). No PDF/email export.
