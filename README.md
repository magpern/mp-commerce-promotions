...
- **Getting Started** admin tab (`?page=mp-commerce-promotions&tab=getting-started`) — onboarding, limitations, workflow; points merchants to Campaign Builder first.
- **Campaign Builder** (`?page=mp-commerce-promotions` or `tab=campaign-builder`) — **recommended default** merchant workflow: guided goals, simple forms, draft creation.
- **Advanced Promotions** (`tab=all`) — expert mode: list, raw JSON rules, orchestration, codes, cart simulation, and per-promotion diagnostics.
...
Output: `/home/magpern/mp-commerce-promotions-staging/build/mp-commerce-promotions-0.3.0-pilot.2.zip`. See [docs/RELEASE_CHECKLIST.md](docs/RELEASE_CHECKLIST.md) and [docs/PILOT_RELEASE_0.3.0_PILOT2.md](docs/PILOT_RELEASE_0.3.0_PILOT2.md). Pushing a `v*` tag runs [.github/workflows/release.yml](.github/workflows/release.yml) to attach the ZIP to a GitHub Release.
