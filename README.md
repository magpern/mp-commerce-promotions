...
- **Demo types supported:** conditions **`minimum_subtotal`**, **`product_quantity`**, **`category_quantity`**, **`logged_in`**, **`first_order`**, **`customer_role`**, **`billing_country`** (ISO codes from Woo billing), **`customer_email_domain`** (email domain from Woo/user); actions **`percentage_discount`** and **`fixed_amount_discount`** (v1 fee sources). Customer/location conditions are raw JSON only (not in Simple Rule Builder v0). **`PromotionPlanner`** selects promotions per application rules; each selected promotion uses only its **first** supported discount action. **Fixed amount** discounts are clamped per fee and **cumulative total ≤ cart subtotal**.
