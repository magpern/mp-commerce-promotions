<?php
/**
 * Condition: minimum total cart quantity across all line items.
 *
...
final class MinimumCartQuantityCondition implements ConditionInterface {

	private int $quantity;

	public function __construct( int $quantity ) {
		if ( $quantity < 1 ) {
			throw new InvalidArgumentException( 'minimum_cart_quantity quantity must be >= 1.' );
		}
		$this->quantity = $quantity;
	}

	public function get_type(): string {
		return RuleTypes::CONDITION_MINIMUM_CART_QUANTITY;
	}
	...
	public function evaluate( EvaluationContext $context ): ConditionResult {
		...
		$observed = array(
			...
			'required_minimum'    => $this->quantity,
		);

		if ( $actual < (float) $this->quantity ) {
			return ConditionResult::fail(
				sprintf(
					'Cart quantity %.4f is below required minimum %d.',
					$actual,
					$this->quantity
				),
