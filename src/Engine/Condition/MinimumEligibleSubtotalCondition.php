<?php
/**
 * Condition: scoped eligible line subtotal must meet a minimum.
 *
...
final class MinimumEligibleSubtotalCondition implements ConditionInterface {
	...
	public static function from_config( array $config ): self {
		if ( ! isset( $config['amount'] ) || ! is_numeric( $config['amount'] ) ) {
			throw new InvalidArgumentException( 'minimum_eligible_subtotal amount is required.' );
		}
	...
	public function __construct(
		...
		if ( $amount < 0 ) {
			throw new InvalidArgumentException( 'minimum_eligible_subtotal amount must be >= 0.' );
		}
	...
	public function get_type(): string {
		return RuleTypes::CONDITION_MINIMUM_ELIGIBLE_SUBTOTAL;
	}
	...
	public function evaluate( EvaluationContext $context ): ConditionResult {
		...
		if ( $eligible_subtotal < $this->amount ) {
			return ConditionResult::fail(
				sprintf(
					'Eligible subtotal %.4f is below required minimum %.4f.',
					$eligible_subtotal,
					$this->amount
				),
				ConditionTrace::REASON_ELIGIBLE_SUBTOTAL_TOO_LOW,
				$observed
			);
		}
